...
> [!NOTE]
> If Internet is unavailable on registry node, upload from file.

    [root@reg01 ~]# oc image mirror -a ${LOCAL_SECRET_JSON} --from-dir=${REMOVABLE_MEDIA_PATH}/mirror "file://openshift/release:${OCP_RELEASE}*" ${LOCAL_REGISTRY}/${LOCAL_REPOSITORY} 
...
![alt text](<images/openshift UI.png>)
...
## Create IDM as LDAP authentication. 
    ...
    spec:
      ...
      - ldap:
          ...
          ca:
            name: ca-config-map
          insecure: false
          url: ldap://idm.ocp4.example.com./cn=accounts,dc=ocp4,dc=example,dc=com?uid
        mappingMethod: claim
    ...
    spec:
      ...
      template:
        ...
        spec:
          ...
          containers:
            - name: nfs-client-provisioner
              image:  registry.k8s.io/sig-storage/nfs-subdir-external-provisioner:latest
              volumeMounts:
...
![alt text](images/nfs-status-bound.png)
